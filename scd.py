	...
	def createMap(self, mapType, extras=False):
		...
		with open(mapType+".txt") as f:
			for line in f.readlines():
				simpleMatch = re.search('([a-zA-Z0-9 \(\)\[\]]+)([0-9]+)? +(#[a-zA-Z0-9]+)', line)
				extrasMatch = re.search('([a-zA-Z \(\)]+) (#[a-zA-Z0-9]+) ([a-zA-Z0-9 \*\#]+)?', line)
 				nbsMatch = re.search('([a-zA-Z ]+) (\#[0-9A-Z]+) (\#[0-9A-Z]+) ([A-Za-z ]+) ?(\*OUT)? ?(\*IGNORE0)? ?(\*\#[0-9A-Z]+.+)?', line)
